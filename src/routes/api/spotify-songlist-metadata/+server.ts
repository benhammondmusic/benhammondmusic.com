	...
	while (songlist_endpoint) {
		...
		for (let i = 0; i < batchArtistIds.length; i += 50) {
			const artistsBatchUrl =
				SEVERAL_ARTISTS_BASE_ENDPOINT + batchArtistIds.slice(i, i + 50).join(",");
			const tracksArtistDetailsJson = await fetch(artistsBatchUrl, headerOptions).then((res) =>
				res.json(),
			);
			// console.log("******");
			// console.log(i, i + 50);
		...
		const songs = jsonData.items.map((item: { track: Song }) => {
			...
			const { danceability, energy, key, mode, acousticness, valence, tempo, id, time_signature } =
				features;
				...
				artists: artists.map((artist) => {
					return {
						...
						genres: batchArtistData.find(
							(artistItem: { id: string; genres: string[] }) => artistItem.id === artist.id,
						)?.genres,
					};
				}),
				danceability,
